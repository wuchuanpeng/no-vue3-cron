  ...
  setup(props, { emit }) {
    ...
    const state = reactive({
      ...
      week: {
        ...
        cronNthDayDay: 1,
        cronNthDayNth: 1,
        eunms: ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT']
      },
      month: {
        cronEvery: "1",
        incrementStart: 3,
    ...
    const parseSeconds = crons => {
      ...
      if (secondsText === '*') {
        ...
        state.second.incrementStart = Number(secondsTexts[0])
        state.second.incrementIncrement = Number(secondsTexts[1])
      } else if (secondsText.includes(',') || isFinite(secondsText)) {
        state.second.cronEvery = '3'
        state.second.specificSpecific = secondsText.split(',').map(item => Number(item))
      } else if (secondsText.includes('-')) {
        state.second.cronEvery = '4'
        let secondsTexts = secondsText.split('-')
        state.second.rangeStart = Number(secondsTexts[0])
        state.second.rangeEnd = Number(secondsTexts[1])
      }
    }

    // 解析minutes
    const parseMinutes = crons => {
      ...
      if (minutesText === '*') {
        ...
        state.minute.incrementStart = Number(minutesTexts[0])
        state.minute.incrementIncrement = Number(minutesTexts[1])
      } else if (minutesText.includes(',') || isFinite(minutesText)) {
        state.minute.cronEvery = '3'
        state.minute.specificSpecific = minutesText.split(',').map(item => Number(item))
      } else if (minutesText.includes('-')) {
        state.minute.cronEvery = '4'
        let minutesTexts = minutesText.split('-')
        state.minute.rangeStart = Number(minutesTexts[0])
        state.minute.rangeEnd = Number(minutesTexts[1])
      }
    }

    // 解析hours
    const parseHours = crons => {
      ...
      if (hoursText === '*') {
        ...
        state.hour.incrementStart = Number(hoursTexts[0])
        state.hour.incrementIncrement = Number(hoursTexts[1])
      } else if (hoursText.includes(',') || isFinite(hoursText)) {
        state.hour.cronEvery = '3'
        state.hour.specificSpecific = hoursText.split(',').map(item => Number(item))
      } else if (hoursText.includes('-')) {
        state.hour.cronEvery = '4'
        let hoursTexts = hoursText.split('-')
        state.hour.rangeStart = Number(hoursTexts[0])
        state.hour.rangeEnd = Number(hoursTexts[1])
      }
    }

    // 解析days weeks
    const parseDaysAndWeeks = crons => {
      let daysText = crons[3].trim()
      let weeksText = crons[5].trim()
      if (daysText.includes('/')) {
        state.day.cronEvery = '3'
        let daysTexts = daysText.split('/')
        state.day.incrementStart = Number(daysTexts[0])
        state.day.incrementIncrement = Number(daysTexts[1])
      } else if (daysText.includes(',') || isFinite(daysText)) {
        state.day.cronEvery = '5'
        state.day.specificSpecific = daysText.split(',').map(item => Number(item))
      } else if (daysText === 'L') {
        ...
        state.day.cronDaysBeforeEomMinus = Number(daysText.replaceAll('L-', ''))
      } else if (daysText.endsWith('W')) {
        state.day.cronEvery = '10'
        state.day.cronDaysNearestWeekday = Number(daysText.replaceAll('W', ''))
      } else if (daysText === '?') {
        if (weeksText.includes('/')) {
          state.day.cronEvery = '2'
          let weeksTexts = weeksText.split('/')
          state.week.incrementStart = Number(weeksTexts[0])
          state.week.incrementIncrement = Number(weeksTexts[1])
        ...
        } else if (state.week.eunms.includes(weeksText)) {
          state.day.cronEvery = '4'
          state.week.specificSpecific = weeksText.split(',').map(item => item)
        } else if (weeksText.includes('#')) {
          state.day.cronEvery = '11'
          let weeksTexts = weeksText.split('#')
          state.week.cronNthDayDay = Number(weeksTexts[0])
          state.week.cronNthDayNth = Number(weeksTexts[1])
        } else if (weeksText.endsWith('L')) {
          state.day.cronEvery = '8'
          state.week.cronLastSpecificDomDay = Number(weeksText.replaceAll('L', ''))
        }
      } else {
    ...
    const parseMonths = crons => {
      ...
      if (monthsText === '*') {
        ...
        state.month.incrementStart = Number(monthsTexts[0])
        state.month.incrementIncrement = Number(monthsTexts[1])
      } else if (monthsText.includes(',') || isFinite(monthsText)) {
        state.month.cronEvery = '3'
        state.month.specificSpecific = monthsText.split(',').map(item => Number(item))
      } else if (monthsText.includes('-')) {
        state.month.cronEvery = '4'
        let monthsTexts = monthsText.split('-')
        state.month.rangeStart = Number(monthsTexts[0])
        state.month.rangeEnd = Number(monthsTexts[1])
      }
    }

    // 解析years
    const parseYears = crons => {
      ...
      if (yearsText === '*') {
        ...
        state.year.incrementStart = Number(yearsTexts[0])
        state.year.incrementIncrement = Number(yearsTexts[1])
      } else if (yearsText.includes(',') || isFinite(yearsText)) {
        state.year.cronEvery = '3'
        state.year.specificSpecific = yearsText.split(',').map(item => Number(item))
      } else if (yearsText.includes('-')) {
        state.year.cronEvery = '4'
        let yearsTexts = yearsText.split('-')
        state.year.rangeStart = Number(yearsTexts[0])
        state.year.rangeEnd = Number(yearsTexts[1])
      }
    }

    function parseCron(crons) {
      parseSeconds(crons)
      parseMinutes(crons)
      parseHours(crons)
      parseDaysAndWeeks(crons)
      parseMonths(crons)
      parseYears(crons)
    }
